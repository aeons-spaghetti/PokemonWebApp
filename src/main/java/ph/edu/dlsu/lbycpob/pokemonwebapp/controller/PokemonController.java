...
public class PokemonController {
    ...
    public String viewRandom(RedirectAttributes redirectAttributes) {
        ...
        if (randomPokemon != null) {
            redirectAttributes.addFlashAttribute("selectedPokemon",
                    randomPokemon);
    ...
    public String search(@RequestParam("name") String name, RedirectAttributes
            redirectAttributes) {
        ...
        if (found != null) {
            ...
            redirectAttributes.addFlashAttribute("message", "Pokemon '" + trimmed
                    + "' not found in the database.");
        }
        return "redirect:/";
    }
    @PostMapping("/pokemon/remove")
    public String remove(@RequestParam("name") String name, RedirectAttributes
            redirectAttributes) {
        String trimmed = name == null ? "" : name.trim();
        if (trimmed.isEmpty()) {
        ...
        if (removed) {
            redirectAttributes.addFlashAttribute("message", "Pokemon '" + trimmed + "' has been removed from the database. Remaining Pokemon: "
